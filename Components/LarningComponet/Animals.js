...
export default function Animals() {
  ...
  async function playSound() {
    const { sound } = await Audio.Sound.createAsync({
      uri: AnimalsData[songIndex].audio,
    });
    setSound(sound);
  ...
  const goNext = async () => {
    slider.current.scrollToOffset({
      offset: ((songIndex + 1) % AnimalsData.length) * width,
    });
    const { sound } = await Audio.Sound.createAsync({
      uri: AnimalsData[(songIndex + 1) % AnimalsData.length].audio,
    });
    setSound(sound);
    await sound.playAsync();
  };
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        <View style={{ marginTop: -80 }}>
          <Text style={styles.artist}>{AnimalsData[songIndex].title}</Text>
        </View>
