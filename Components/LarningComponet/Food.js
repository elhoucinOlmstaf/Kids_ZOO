...
import FoodData from '../../DataBase/DummyData/FoodDat'
...
export default function Food() {
  ...
  const goNext = async () => {
    slider.current.scrollToOffset({
      offset: ((songIndex + 1) % FoodData.length) * width,
    });
    const { sound } = await Audio.Sound.createAsync({
      uri: FoodData[(songIndex + 1) % FoodData.length].audio,
    });
    setSound(sound);
    await sound.playAsync();
  };
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        <View style={{ marginTop: -80 }}>
          <Text style={styles.artist}>{FoodData[songIndex].title}</Text>
        </View>
